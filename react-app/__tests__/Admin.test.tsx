...
describe("Admin auth gates", () => {
  ...
  it("rejects non-spike users", () => {
    renderWithProviders(<Admin />, {
      preloadedState: {
        user: { access: "fake-token", refresh: "r", username: "testUser" },
      },
    });
...
describe("Asset Load", () => {
  ...
  it("sends overwriteExisting=true when the switch is on", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Asset Load" }));
    expect(
      await screen.findByText("asset-load ok?overwriteExisting=true"),
    ).toBeInTheDocument();
  });
  ...
  it("falls back to the legacy load endpoint on 404", async () => {
    server.use(
      http.get(
        springbootUrl.concat("admin/asset-load"),
  ...
  it("surfaces the fallback error when both endpoints fail", async () => {
    server.use(
      ...
      http.get(
        springbootUrl.concat("admin/load"),
    ...
    await userEvent.click(screen.getByRole("button", { name: "Asset Load" }));
    expect(
      await screen.findByText("legacy loader exploded"),
    ).toBeInTheDocument();
  });
});
...
describe("Sync Frames", () => {
  ...
  it("explains 401 responses with the shared-secret hint", async () => {
    server.use(
      http.get(
        springbootUrl.concat("admin/sync-frames"),
...
describe("Download IEX HIST", () => {
  it("sends the entered number of trading days", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Download HIST" }),
    );
...
describe("Adjust Prices", () => {
  it("sends ticker, force, etfOnly, and minConfidence", async () => {
    ...
    await userEvent.type(
      screen.getAllByPlaceholderText("e.g. AAPL")[0],
      "aapl",
    );
    await userEvent.click(getSwitches()[1]); // force re-fetch
    await userEvent.click(getSwitches()[2]); // ETF only
    await userEvent.click(
      screen.getByRole("button", { name: "Adjust Prices" }),
    );

    expect(
      await screen.findByText(
        "adjust-prices ok?ticker=AAPL&force=true&etfOnly=true&minConfidence=70",
      ),
    ).toBeInTheDocument();
  });
...
describe("Summarize Filings", () => {
  it("sends the uppercased ticker", async () => {
    ...
    await userEvent.type(
      screen.getAllByPlaceholderText("e.g. AAPL")[1],
      "msft",
    );
    await userEvent.click(
      screen.getByRole("button", { name: "Summarize Filings" }),
    );
    expect(
      await screen.findByText("summarize-filings ok?ticker=MSFT"),
    ).toBeInTheDocument();
  });
});
...
describe("Refresh index metrics", () => {
  it("sends no params for the default russell1000 scope", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Refresh index metrics" }),
    );
  ...
  it("sends scope=all when selected", async () => {
    ...
    expect(
      await screen.findByText("refresh-stocks ok?scope=all"),
    ).toBeInTheDocument();
  });
  ...
  it("a ticker wins over scope and disables the scope select", async () => {
    ...
    expect(
      await screen.findByText("refresh-stocks ok?ticker=NFLX"),
    ).toBeInTheDocument();
  });
});
...
describe("Rebuild indexes", () => {
  it("sends no params by default (all indexes)", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Rebuild index(es)" }),
    );
  ...
  it("sends refreshMetrics and the selected code", async () => {
    ...
    expect(
      await screen.findByText("rebuild ok?refreshMetrics=true&code=FAT100"),
    ).toBeInTheDocument();
  });
});
...
describe("Admin navigation", () => {
  it("navigates to the success bar page", async () => {
    renderWithRoute(<Admin />, {
      ...
      extraRoutes: [
        {
          path: "/react-admin/success-bar",
          element: <div>success-bar-probe</div>,
        },
      ],
    });
