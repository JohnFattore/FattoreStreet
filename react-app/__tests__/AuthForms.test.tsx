...
describe('WatchListForm', () => {
    it('adds a ticker after the quote lookup succeeds', async () => {
        const { store } = renderWithProviders(<WatchListForm assetInfosLoading={false} />);

        await userEvent.type(screen.getByPlaceholderText('Enter Ticker Here'), 'VTI');
    ...
    it('sets an error when the quote has no price', async () => {
        server.use(
            http.get(djangoBaseUrl.concat("portfolio/api/quote/"), () =>
                Response.json({ price: null, percent_change_daily: null }, { status: 200 })
            )
        );

        const { store } = renderWithProviders(<WatchListForm assetInfosLoading={false} />);

        await userEvent.type(screen.getByPlaceholderText('Enter Ticker Here'), 'NOPE');
        await userEvent.click(screen.getByRole('button', { name: /add to watchlist/i }));

        await waitFor(() => {
            expect(store.getState().watchList.error).toBe("Couldn't retrieve data for ticker");
        });
