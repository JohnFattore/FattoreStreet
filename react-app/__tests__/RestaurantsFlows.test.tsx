...
const restaurantsApiBaseUrl = ((import.meta.env.VITE_APP_DJANGO_URL || "").endsWith("/")
...
describe('RestaurantRecommend (dormant feature)', () => {
    it('shows the fetch button until clicked, then renders recommendations', async () => {
        ...
        await userEvent.click(screen.getByRole('button', { name: 'Click for Recommendations' }));

        expect(await screen.findByText('Recommended Restaurants')).toBeInTheDocument();
...
describe('LocationForm', () => {
    it('updates the location slice on submit', async () => {
        ...
        await userEvent.click(screen.getByRole('button', { name: 'Update Location' }));

        await waitFor(() => {
            expect(store.getState().location).toMatchObject({ state: 'CA', city: 'San Diego' });
        });
    });
    ...
    it('requires a city', async () => {
        ...
        expect(await screen.findByText(/City field is required/)).toBeInTheDocument();
...
describe('Restaurants page', () => {
    it('gates logged-out users but still shows the public restaurant table', async () => {
        ...
        expect(screen.getByText('Nashville Restaurant Explorer')).toBeInTheDocument();
