...
describe("Home", () => {
  it("renders the hero section", () => {
    ...
    expect(
      screen.getByText("Master Your Financial Future"),
    ).toBeInTheDocument();
  });
...
describe("SECData", () => {
  ...
  it("renders income statement and balance sheet sections", async () => {
    ...
    expect(
      await screen.findByText("Financial Ratios & Metrics"),
    ).toBeInTheDocument();
  });
  ...
  it("renders quarterly data table", async () => {
    ...
    expect(
      await screen.findByText("Historical Quarterly Data"),
    ).toBeInTheDocument();
  });
  ...
  it("renders the filing summaries section", async () => {
    ...
    expect(
      await screen.findByText("10-K Filing Summaries"),
    ).toBeInTheDocument();
  });
});
...
describe("Register", () => {
  it("renders the registration heading and form fields", () => {
    ...
    expect(
      screen.getByText("Register for a Fattore Account"),
    ).toBeInTheDocument();
  ...
  it("shows Fattore 1000 vs IWB comparison when FAT1000 is selected", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: /Fattore 1000 vs Russell 1000/i,
      }),
    ).toBeInTheDocument();
...
describe("AdminSuccessBar", () => {
  it("renders page title and empty-state table message", () => {
    ...
    expect(
      screen.getByText("Corporate Action Success Bar"),
    ).toBeInTheDocument();
...
describe("Entertainment", () => {
  ...
  it("renders external links with hrefs", () => {
    renderWithProviders(<Entertainment />);
    expect(
      screen.getByRole("link", { name: "S&P 500 Heat Map" }),
    ).toHaveAttribute("href", "https://finviz.com/map.ashx");
    expect(
      screen.getByRole("link", { name: "Stock Screener" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders ratio rows with formulas", () => {
    ...
    expect(
      screen.getByText("Market Capitalization / Annual Income"),
    ).toBeInTheDocument();
...
describe("IexPricesView", () => {
  it("renders daily price table for a ticker", async () => {
    ...
    expect(
      await screen.findByText("AAPL — IEX Daily Prices"),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(
        "3 trading days of adjusted OHLCV data from IEX exchange prices.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders adjusted price comparison column headers", async () => {
    ...
    expect(
      await screen.findByRole("columnheader", { name: "IEX Adjusted Close" }),
    ).toBeInTheDocument();
    expect(
      await screen.findByRole("columnheader", {
        name: "YFinance Adjusted Close",
      }),
    ).toBeInTheDocument();
    expect(
      (await screen.findAllByRole("columnheader", { name: "Difference" }))
        .length,
    ).toBeGreaterThan(0);
    expect(
      await screen.findByRole("columnheader", { name: "% Difference" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders the price comparison section", async () => {
    ...
    expect(
      await screen.findByText("IEX vs YFinance Adjusted Price Comparison"),
    ).toBeInTheDocument();
  });
  ...
  it("renders the dividend comparison section", async () => {
    ...
    expect(
      await screen.findByText("Internal vs YFinance Dividend Comparison"),
    ).toBeInTheDocument();
  });
  ...
  it("renders the split comparison section", async () => {
    ...
    expect(
      await screen.findByText("Internal vs YFinance Split Comparison"),
    ).toBeInTheDocument();
  });
});
