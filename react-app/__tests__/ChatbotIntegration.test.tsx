...
describe("ChatbotOutput", () => {
  it("renders user and model messages correctly", async () => {
    ...
    renderWithProviders(<ChatbotOutput />, {
    ...
    server.use(
      http.get(chatbotUrl, () =>
        Response.json(
          [
            {
              input_text: "Any advice?",
              output_text: "Buy **index funds** and hold",
              timestamp: "2026-03-01T00:00:00Z",
            },
          ],
          { status: 200 },
  ...
  it("renders empty state when no messages exist", async () => {
    server.use(http.get(chatbotUrl, () => Response.json([], { status: 200 })));
...
describe("Chatbot send flow", () => {
  it("optimistically shows the user message, then appends the model reply", async () => {
    server.use(
      http.get(chatbotUrl, () =>
        Response.json(
          [
            {
              input_text: "Earlier question",
              output_text: "Earlier answer",
              timestamp: "2026-03-01T00:00:00Z",
            },
          ],
          { status: 200 },
    ...
    expect(
      await screen.findByText(/Stay the course and keep costs low/),
    ).toBeInTheDocument();
  });
});
