...
const columns = [
    { label: 'Name', sortKey: 'name' as const, render: (row: typeof sampleData[0]) => row.name },
    { label: 'Price', sortKey: 'price' as const, render: (row: typeof sampleData[0]) => `$${row.price}` },
    { label: 'Ticker', sortKey: 'ticker' as const, render: (row: typeof sampleData[0]) => row.ticker },
];

describe('SortableTable', () => {
    it('renders column headers and data rows', () => {
        renderWithProviders(
            <SortableTable data={sampleData} columns={columns} isLoading={false} errors={[]} />
        );
    ...
    it('handles empty data', () => {
        renderWithProviders(
            <SortableTable data={[]} columns={columns} isLoading={false} errors={[]} />
        );
    ...
    it('shows spinner when loading', () => {
        const { container } = renderWithProviders(
            <SortableTable data={[]} columns={columns} isLoading={true} errors={[]} />
        );
...
describe('StateHandler', () => {
    it('renders content when not loading and no errors', () => {
        renderWithProviders(
            <StateHandler content={<p>Hello World</p>} />
        );
