...
describe('RestaurantTable', () => {
    ...
    it('filters restaurants with the search bar', async () => {
        ...
        await userEvent.type(screen.getByPlaceholderText('Restaurant Search'), 'zzz');
    ...
    it('sorts rows when a column header is clicked', async () => {
        ...
        expect(within(rows[1]).getByText("Arnold's Country Kitchen")).toBeInTheDocument();
    ...
    it('calls setRestaurant when Create Review is clicked', async () => {
        ...
        await userEvent.click(await screen.findByRole('button', { name: /create review/i }));
        expect(setRestaurant).toHaveBeenCalledWith(expect.objectContaining({ name: 'Sonic Drive-In' }));
    });
});
...
describe('ReviewTable', () => {
    ...
    it("renders the user's reviews", async () => {
        renderWithProviders(<ReviewTable />, { preloadedState: authenticatedState });
    ...
    it('removes a review after deleting it', async () => {
        ...
        server.use(
            ...
            http.delete(new RegExp(restaurantsApiBaseUrl.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "review/\\d+/"), () => {
                deleted = true;
        ...
        expect(await screen.findByText('testUser has no reviews')).toBeInTheDocument();
    });
});
...
describe('ReviewForm', () => {
    it('submits a review and shows the success message', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Review for Sonic Drive-In submitted')).toBeInTheDocument();
        });
    });
    ...
    it('shows the API error when the review is rejected', async () => {
        ...
        expect(await screen.findByText('You have already reviewed this restaurant.')).toBeInTheDocument();
