...
import userReducer, { logout, setUserDarkMode } from "../src/reducers/userReducer";
import locationReducer, { setLocation } from "../src/reducers/locationReducer";
import watchListReducer, { loadTickers, removeTicker } from "../src/reducers/watchListReducer";
import adminSuccessBarReducer, { addTicker, clearError, removeTicker as removeAdminTicker } from "../src/reducers/adminSuccessBarReducer";
...
describe("userReducer mutation matchers", () => {
    it("refreshLogin success replaces access and keeps refresh", async () => {
        ...
        await store.dispatch(djangoApi.endpoints.refreshLogin.initiate("old-refresh"));
    ...
    it("login failure clears the refresh token", async () => {
        ...
        await store.dispatch(
            djangoApi.endpoints.login.initiate({ username: "testUser", password: "wrong" }) // pragma: allowlist secret
        );
...
describe("locationReducer", () => {
    it("setLocation replaces state and city", () => {
        const state = locationReducer(undefined, setLocation({ state: "CA", city: "San Diego" }));
...
describe("watchListReducer", () => {
    it("loadTickers seeds default tickers into empty localStorage", () => {
        ...
        expect(JSON.parse(localStorage.getItem("tickers") as string)).toEqual(["VTI", "SPY"]);
    });
    ...
    it("removeTicker updates state and localStorage", () => {
        ...
        expect(JSON.parse(localStorage.getItem("tickers") as string)).toEqual(["SPY"]);
    });
});

describe("adminSuccessBarReducer validation branches", () => {
    it("rejects an empty ticker", () => {
        const state = adminSuccessBarReducer({ tickers: [], error: "" }, addTicker("   "));
        expect(state.tickers).toEqual([]);
        expect(state.error).toBe("Ticker is required.");
    });

    it("rejects an invalid ticker format", () => {
        const state = adminSuccessBarReducer({ tickers: [], error: "" }, addTicker("1BAD$"));
        expect(state.tickers).toEqual([]);
