...
describe("AccountList", () => {
  it("renders account names and calculates balances correctly", async () => {
    ...
    expect(
      await screen.findByText(/-\$40\.40 \(-1\.00%\)/),
    ).toBeInTheDocument();
  });
});
...
describe("AssetTable", () => {
  it("shows Loading... while asset info loads, then resolves", async () => {
    server.use(
      http.get(`${portfolioApiBaseUrl}asset-info/`, async () => {
        await delay(500);
    ...
    expect(
      await screen.findByText("Microsoft Corporation"),
    ).toBeInTheDocument();
...
describe("AssetTickerTable", () => {
  it("shows Loading... while asset info loads, then resolves with price", async () => {
    ...
    renderWithProviders(<AssetTickerTable ticker="AAPL" />, {
      preloadedState: authenticatedState,
    });
