...
describe("AssetView", () => {
  ...
  it("renders the ticker header, price chart card, and navigation buttons", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "SEC EDGAR Data" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "IEX Price History" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Back to Portfolio" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Back to WatchList" }),
    ).toBeInTheDocument();
  });
  ...
  it("navigates to SEC EDGAR data for the ticker", async () => {
    renderAssetView([
      { path: "/sec-edgar/:ticker", element: <div>sec-edgar-probe</div> },
    ]);

    await userEvent.click(
      await screen.findByRole("button", { name: "SEC EDGAR Data" }),
    );
  ...
  it("navigates back to the portfolio", async () => {
    renderAssetView([
      { path: "/portfolio", element: <div>portfolio-probe</div> },
    ]);

    await userEvent.click(
      await screen.findByRole("button", { name: "Back to Portfolio" }),
    );
