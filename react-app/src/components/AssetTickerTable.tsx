...
function AssetRow({ asset, assetInfo }) {
  ...
  const [deleteAsset, { error, isLoading }] =
    useDeleteAssetMutation();
  ...
  return (
    <>
      ...
      <Modal show={showDelete} onHide={handleCloseDelete}>
        ...
        <Modal.Body>
          {error ? <Alert variant="danger">{getErrorMessages(error["data"])}</Alert> : null} 
          {`Would you like to delete ${asset?.ticker}?`}</Modal.Body>
        <Modal.Footer>
      ...
      <tr>
        ...
        <td>{asset.sellDate || "Not Sold"}</td>
        <td>
          {asset.sellPrice
            ? formatString(asset.sellPrice, "money")
            : "Not Sold"}
        </td>
        <td>
