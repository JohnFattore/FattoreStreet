...
const COMPARE_FIELDS = [
  ...
  {
    label: "Net Income",
    secKey: "netIncomeLoss",
    yfKey: "netIncomeLoss",
    format: "money",
  },
  {
    label: "Operating Income",
    secKey: "operatingIncomeLoss",
    yfKey: "operatingIncomeLoss",
    format: "money",
  },
  {
    label: "Gross Profit",
  ...
  },
  {
    label: "EPS Basic",
    secKey: "epsBasic",
    yfKey: "earningsPerShareBasic",
    format: "eps",
  },
  {
    label: "EPS Diluted",
    secKey: "epsDiluted",
    yfKey: "earningsPerShareDiluted",
    format: "eps",
  },
  ...
  },
  {
    label: "Equity",
    secKey: "equity",
    yfKey: "stockholdersEquity",
    format: "money",
  },
  {
    label: "Cash",
    secKey: "cash",
    yfKey: "cashAndCashEquivalentsAtCarryingValue",
    format: "money",
  },
  {
    label: "Receivables",
    secKey: "receivables",
    yfKey: "accountsReceivableNetCurrent",
    format: "money",
  },
  {
    label: "Inventory",
  ...
  },
  {
    label: "OCF",
    secKey: "ocf",
    yfKey: "netCashProvidedByUsedInOperatingActivities",
    format: "money",
  },
  {
    label: "Dividends",
    secKey: "dividends",
    yfKey: "paymentsOfDividends",
    format: "money",
  },
  {
    label: "Buybacks",
    secKey: "buybacks",
    yfKey: "paymentsForRepurchaseOfCommonStock",
    format: "money",
  },
] as const;
...
export default function QuarterlyComparison({ ticker }: { ticker: string }) {
  const {
    ...
    error: secError,
  } = useGetSecQuartersQuery(ticker);
  const {
    data: yfData,
    isLoading: yfLoading,
    error: yfError,
  } = useGetDjangoQuartersQuery(ticker);
  ...
  const rows = useMemo(() => {
    ...
    for (const key of allKeys) {
      ...
      const yfVal: number | null =
        yfQ != null
          ? yfMetric(yfQ, field.yfKey as keyof IYFinanceQuarter)
          : null;
    ...
    result.sort((a, b) =>
      b.year !== a.year ? b.year - a.year : b.qtr - a.qtr,
    );
    return result;
              ...
              <>
                ...
                <Table hover size="sm" responsive>
                  ...
                  <tbody>
                    {rows.map((row) => {
                      const isMismatch =
                        row.pctDiff != null && Math.abs(row.pctDiff) > 1;
                      return (
                        <tr
                          key={row.period}
                        ...
                        >
                          ...
                          <td
                            className={isMismatch ? "text-danger fw-bold" : ""}
                          >
                    ...
                    {rows.length === 0 && (
                      <tr>
                        <td colSpan={5}>No overlapping quarters found.</td>
                      </tr>
                    )}
                  </tbody>
                </Table>
