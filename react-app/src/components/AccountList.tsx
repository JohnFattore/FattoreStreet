...
export default function AccountList() {
  ...
  const { data: assetInfos, isLoading: assetInfosLoading } =
    useGetAssetInfosQuery(uniqueTickers, {
                ...
                {accounts.map((account) => {
                  ...
                  if (assets && assetInfos) {
                    const accountAssets = assets.filter(
                      (a: IAsset) => a.account === account.id,
                    ); // 'account' is the field name from django
                    ...
                    accountAssets.forEach((asset: IAsset) => {
                      ...
                      if (info && !asset.sellDate) {
                        ...
                        const previousPrice =
                          info.currentPrice / (1 + info.percentChangeDaily);
                  ...
                  const dayChangePercent =
                    previousBalance > 0 ? dayChange / previousBalance : 0;
                    ...
                    <tr key={account.id}>
                      ...
                      >
                        {isDataLoading ? (
                          ...
                          <>
                            {formatString(dayChange, "money")} (
                            {formatString(dayChangePercent, "percent")})
                          </>
                        )}
                      </td>
                      <td>
