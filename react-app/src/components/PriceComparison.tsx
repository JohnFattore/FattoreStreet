...
export default function PriceComparison({ ticker }: { ticker: string }) {
  const {
    data: iexData,
    isLoading: iexLoading,
    error: iexError,
  } = useGetIexPricesQuery(ticker);
  const {
    data: yfData,
    isLoading: yfLoading,
    error: yfError,
  } = useGetAssetPricesQuery(ticker);
  ...
  const overlapSummary = useMemo(() => {
    ...
    const buildMetric = (limitPct: number) => {
      const count = overlappingRows.filter(
        (row) => row.pctDiff != null && Math.abs(row.pctDiff) <= limitPct,
      ).length;
      const pct =
        totalOverlapping > 0 ? (count / totalOverlapping) * 100 : null;
      return { count, pct };
    };
              ...
              <>
                ...
                <Table hover size="sm" responsive>
                  ...
                  <tbody>
                    ...
                    {rows.length === 0 && (
                      <tr>
                        <td colSpan={5}>No overlapping dates found.</td>
                      </tr>
                    )}
                  </tbody>
                </Table>
