...
export default function AssetView() {
  ...
  return (
    <>
      ...
      <Row>
        <Col>
          <h3>Purchased Asset</h3>
          <ListGroup>
            <ListGroup.Item>{`Current total price is ${formatString(
              asset.currentPrice,
              "money"
            )}`}</ListGroup.Item>
            <ListGroup.Item>{`${formatString(
              asset.shares,
              "amount"
            )} shares of ${asset.ticker} were bought on ${asset.buyDate} for ${formatString(
              asset.costBasis,
              "money"
            )}`}</ListGroup.Item>
            <ListGroup.Item>{`The percent change is ${formatString(
              asset.percentChange,
              "percent"
            )}, compared to the S&P 500 which changed ${formatString(
              asset.snp500PercentChange,
              "percent"
            )} in the same time`}</ListGroup.Item>
            {asset.sellPrice ? (
              <ListGroup.Item>{`${asset.ticker} sold for ${formatString(
                asset.sellPrice,
                "money"
              )} on ${asset.sellDate}`}</ListGroup.Item>
            ) : null}
          </ListGroup>
        </Col>
        <Col>
          <h3>General Financials</h3>
          <StockFinancials ticker={asset.ticker} />
        </Col>
      </Row>
      ...
      <Button onClick={handleShowDelete}>{`Delete ${asset.ticker}`}</Button>
