...
import {
  ...
  removeTicker,
  setValidationError,
} from "../reducers/adminSuccessBarReducer";
...
const calculatePriceSummary = (
  ...
  for (const [date, springValue] of iexMap.entries()) {
    ...
    const pctDiff =
      (Math.abs(yfValue - springValue) / Math.abs(springValue)) * 100;
    overlappingDates++;
...
const calculateDividendSummary = (
  ...
  for (const springRow of sortedSpring) {
    ...
    if (bestIndex >= 0) {
      ...
      matchedCount++;
      if (
        Math.abs(sortedYf[bestIndex].value - springRow.value) <=
  ...
  const valueAgreementRate =
    matchedCount > 0 ? matchedWithinValueTolerance / matchedCount : 0;
  ...
  const dividendSummary = useMemo(
    () =>
      calculateDividendSummary(
        springDividends.data?.dividends ?? [],
        yfDividends.data ?? [],
      ),
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="small text-muted">
              1%:{" "}
              {formatRatio(
                priceSummary.within1Pct,
                priceSummary.overlappingDates,
              )}{" "}
              | 3%:{" "}
              {formatRatio(
                priceSummary.within3Pct,
                priceSummary.overlappingDates,
              )}{" "}
              | 5%:{" "}
              {formatRatio(
                priceSummary.within5Pct,
                priceSummary.overlappingDates,
              )}{" "}
              | 25%:{" "}
              {formatRatio(
                priceSummary.within25Pct,
                priceSummary.overlappingDates,
              )}
            </div>
            <div>
              <Badge bg={getScoreVariant(priceSummary.score)}>
                {formatPercent(priceSummary.score)}
              </Badge>
            </div>
          </>
        )}
        ...
        {isLoading ? (
          ...
          <>
            <div>
              Matches:{" "}
              {formatRatio(
                dividendSummary.matchedCount,
                Math.max(dividendSummary.springCount, dividendSummary.yfCount),
              )}
            </div>
            <div className="small text-muted">
              Value-aligned:{" "}
              {formatRatio(
                dividendSummary.matchedWithinValueTolerance,
                Math.max(dividendSummary.matchedCount, 1),
              )}
            </div>
            <div>
              <Badge bg={getScoreVariant(dividendSummary.score)}>
                {formatPercent(dividendSummary.score)}
              </Badge>
            </div>
          </>
        )}
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="small text-muted mt-1">
              Based on within-1% adjusted-price matches only
            </div>
          </>
        )}
      </td>
...
export default function AdminSuccessBar() {
  ...
  const { tickers, error } = useSelector(
    (state: RootState) => state.adminSuccessBar,
  );
  const [tickerInput, setTickerInput] = useState("");
  const assetInfoArg: string[] | typeof skipToken =
    tickers.length > 0 ? tickers : skipToken;
  ...
  return (
    <div className="admin-page">
      ...
      <p className="text-muted">
        Compare Spring Boot adjusted prices/dividends against yfinance by
        ticker. Success percentage is based only on within-1% adjusted-price
        matches.
      </p>
