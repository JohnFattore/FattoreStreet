...
export default function Admin() {
  ...
  const [summarizeFilings, summarizeFilingsState] =
    useAdminSummarizeFilingsMutation();
  const [refreshIndexMetrics, refreshIndexMetricsState] =
    useAdminRefreshIndexMetricsMutation();
  const [rebuildIndexes, rebuildIndexesState] =
    useAdminRebuildIndexesMutation();
  ...
  const [indexMetricsScope, setIndexMetricsScope] = useState<
    "russell1000" | "all"
  >("russell1000");
  const [indexMetricsTicker, setIndexMetricsTicker] = useState("");
  const [indexRebuildCode, setIndexRebuildCode] = useState("");
  const [indexRebuildRefreshMetricsFirst, setIndexRebuildRefreshMetricsFirst] =
    useState(false);
  ...
  return (
    <div className="admin-page">
      ...
      <div className="mb-3">
        <Button
          onClick={() => navigate("/react-admin/success-bar")}
      ...
      <Card>
        ...
        <p className="text-muted small mb-3">
          Uses your Django login access token (
          <code>Authorization: Bearer</code>). Only Django user id 1 can invoke
          these routes on the SEC API.
        </p>
        ...
        <Card>
          ...
          <p>
            Fetches all US tickers from SEC endpoints and enriches ETF listings
            with SEC series/class identity data.
          </p>
          <p className="text-muted small mb-2">
            <strong>Affects (springboot DB):</strong> <code>Asset</code>,{" "}
            <code>Listing</code>
          </p>
          <Form.Check
            className="mb-2"
          ...
          <Button
            onClick={() =>
              assetLoad({ overwriteExisting: loadOverwriteExisting })
            }
            disabled={assetLoadState.isLoading}
          >
            {assetLoadState.isLoading ? (
              <>
                <Spinner size="sm" />
                Loading...
              </>
          ...
          {assetLoadState.data && (
            <Alert variant="success">{assetLoadState.data}</Alert>
          )}
          {assetLoadState.error ? (
            <Alert variant="danger">{formatError(assetLoadState.error)}</Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p className="text-muted small mb-2">
            <strong>Affects (springboot DB):</strong> <code>Quarter</code> and
            related SEC frame data
          </p>
          <Button
            onClick={() => syncFrames()}
            disabled={syncFramesState.isLoading}
          >
            {syncFramesState.isLoading ? (
              <>
                <Spinner size="sm" />
                Syncing...
              </>
          ...
          {syncFramesState.data && (
            <Alert variant="success">{syncFramesState.data}</Alert>
          )}
          {syncFramesState.error ? (
            <Alert variant="danger">{formatError(syncFramesState.error)}</Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p>
            Download IEX TOPS PCAPs, parse trades, and load OHLCV directly into
            the database.
          </p>
          <p className="text-muted small mb-2">
          ...
          {loadHistState.data && (
            <Alert variant="success">{loadHistState.data}</Alert>
          )}
          {loadHistState.error ? (
            <Alert variant="danger">{formatError(loadHistState.error)}</Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p>
            Detect splits and dividends from SEC EDGAR and apply adjustment
            factors to OHLCV prices.
          </p>
          <p className="text-muted small mb-2">
          ...
          <Button
            onClick={() =>
              adjustPrices({
                ticker: adjustTicker.trim()
                  ? adjustTicker.trim().toUpperCase()
                  : undefined,
          ...
          >
            {adjustPricesState.isLoading ? (
              <>
                <Spinner size="sm" /> Adjusting...
              </>
            ) : (
              "Adjust Prices"
            )}
          </Button>
          {adjustPricesState.data && (
            <Alert variant="success">{adjustPricesState.data}</Alert>
          )}
          {adjustPricesState.error ? (
            <Alert variant="danger">
              {formatError(adjustPricesState.error)}
            </Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p>
            Fetch 10-K filings from SEC EDGAR and generate MD&A summaries via
            the local LLM.
          </p>
          <p className="text-muted small mb-2">
          ...
          <Button
            onClick={() =>
              summarizeFilings({
                ticker: summaryTicker.trim()
                  ? summaryTicker.trim().toUpperCase()
                  : undefined,
              })
            }
            disabled={summarizeFilingsState.isLoading}
          >
            {summarizeFilingsState.isLoading ? (
              <>
                <Spinner size="sm" /> Summarizing...
              </>
            ) : (
              "Summarize Filings"
            )}
          </Button>
          {summarizeFilingsState.data && (
            <Alert variant="success">{summarizeFilingsState.data}</Alert>
          )}
          {summarizeFilingsState.error ? (
            <Alert variant="danger">
              {formatError(summarizeFilingsState.error)}
            </Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p>
            Recompute per-listing market cap, float, and volume fields from IEX
            daily prices and SEC companyfacts (run after listings and prices
            exist).
          </p>
          <p className="text-muted small mb-2">
            <strong>Affects (springboot DB):</strong>{" "}
            <code>ListingIndexMetrics</code> (reads <code>Listing</code>,{" "}
            <code>Asset</code>, <code>DailyPrice</code>)
          </p>
          <Form.Group className="mb-2">
            ...
            <Form.Select
              ...
              onChange={(e) =>
                setIndexMetricsScope(e.target.value as "russell1000" | "all")
              }
              disabled={indexMetricsTicker.trim().length > 0}
              style={{ maxWidth: 360 }}
            >
              <option value="russell1000">
                Russell 1000 (IWB holdings file)
              </option>
          ...
          <Button
            onClick={() =>
              refreshIndexMetrics({
                ...
                ticker: indexMetricsTicker.trim()
                  ? indexMetricsTicker.trim().toUpperCase()
                  : undefined,
              })
            }
          ...
          {refreshIndexMetricsState.data && (
            <Alert variant="success">{refreshIndexMetricsState.data}</Alert>
          )}
          {refreshIndexMetricsState.error ? (
            <Alert variant="danger">
              {formatError(refreshIndexMetricsState.error)}
            </Alert>
          ) : null}
        </Card>
        ...
        <Card>
          ...
          <p>
            Russell-style float-adjusted cap weights: upserts{" "}
            <code>MarketIndex</code> and replaces <code>IndexMember</code> rows
            from <code>ListingIndexMetrics</code>. Choose one index or{" "}
            <strong>All</strong> to rebuild every configured index (order:{" "}
            <code>FAT100</code>, <code>FAT1000</code>, <code>FAT50</code>). Not
            an official FTSE Russell index.
          </p>
          <p className="text-muted small mb-2">
            <strong>Affects (springboot DB):</strong> <code>MarketIndex</code>,{" "}
            <code>IndexMember</code>
          </p>
          <Form.Group className="mb-2">
          ...
          <Form.Check
            ...
            onChange={(e) =>
              setIndexRebuildRefreshMetricsFirst(e.target.checked)
            }
          />
          <Button
            onClick={() =>
          ...
          {rebuildIndexesState.data && (
            <Alert variant="success">{rebuildIndexesState.data}</Alert>
          )}
          {rebuildIndexesState.error ? (
            <Alert variant="danger">
              {formatError(rebuildIndexesState.error)}
            </Alert>
          ) : null}
        </Card>
      </Card>
