...
| `/blog`                    | Blog               | The author's own posts, with the `LLM Notebook` category excluded and a link out to it                                                     |
| `/blog/llm-notebook`       | LlmNotebook        | The `LLM Notebook` learning topics, written by (and credited to) Claude                                                                    |
| `/blog/:slug`              | BlogPost           | A single post, Markdown rendered via `react-markdown`                                                                                      |
...
| `BlogPostList`                       | Blog, LlmNotebook      | Post cards for one category filter (`category` / `excludeCategory`)                                                    |
...
- **Spring Boot** (`VITE_APP_SPRINGBOOT_URL`): SEC EDGAR fact sheets, quarters, IEX prices, dividends, splits, filing summaries, indexes (`/indexes`, `/index-members` with nested `stock` including `stateHQ` / `stateIncorp`), IWB reference weights (`/iwb-reference-holdings`), FRED economic data (`POST /fred-data`). All read-only and unauthenticated: the service verifies no tokens, and the admin-job mutations that used to live here were removed with the `/admin/**` routes -- those jobs now run as scheduled Fargate tasks.
