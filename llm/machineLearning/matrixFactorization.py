...
for epoch in range(num_epochs):
    for batch in data_loader:  # Iterate over DataLoader batches
        user_ids, restaurant_ids, ratings = batch
        optimizer.zero_grad()
        predictions = model(user_ids, restaurant_ids)
...
torch.save(model.state_dict(), 'matrixFactorModel.pth')
'''

model.load_state_dict(torch.load('matrixFactorModel.pth', weights_only=True))
...
num_epochs = 50  # You don't need many epochs for a small dataset
for epoch in range(num_epochs):
    optimizer.zero_grad()
...
for key, value in top_5_items:
    print(f"Restaurant: {restaurant_id_to_name[int_to_restaurant_id[key]]}, Value: {value}")