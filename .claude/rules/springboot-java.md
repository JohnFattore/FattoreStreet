...
- A regex over SEC filing text that pairs a lazy or bounded quantifier with a large alternation (the corporate-action date patterns, e.g. `.{0,900}?` before a month-name list) must match through `BoundedRegexInput` (`corporateaction/support/`), which aborts a match that exceeds its budget. That shape backtracks badly and is what FindSecBugs reports as REDOS; the exclusion for it in `config/spotbugs/exclude.xml` is package-wide, so a new one is suppressed rather than reported and will not fail the build. Simple patterns without an alternation do not need the guard. Do not "fix" the shape with possessive quantifiers — the lazy `?` deliberately finds the *nearest* date, and possessive matching changes which date a filing resolves to
