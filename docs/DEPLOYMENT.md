...
- Postgres publishes `5432` on all interfaces: the nightly hist-load Fargate task (`springboot/deploy/terraform/`) connects over the VPC at the EC2 host's private IP, and the EC2 security group only admits that task's SG on 5432. Redis has no host port at all (no auth — debug with `docker exec -it redis redis-cli`); on-host apps connect container-to-container over `dockerNet`.
